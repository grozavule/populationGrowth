...
const nbYear = (p0, growthRate, aug, p, numYears = 0) => {
    let growthRateDecimal = 1 + (growthRate/100);
    let currentPopulation = (p0 * growthRateDecimal) + aug;
    console.log(currentPopulation, numYears);
    return (currentPopulation >= p) ? ++numYears : nbYear(currentPopulation, growthRate, aug, p, ++numYears);
}
